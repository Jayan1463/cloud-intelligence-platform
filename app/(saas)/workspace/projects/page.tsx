    ...
    <section className="space-y-4">
      ...
      <div className="space-y-3">
        {WORKSPACE_PROJECTS.map((project) => {
          ...
          return (
            <div key={project.id} className={`rounded-md border p-4 ${isActive ? "border-cyan-400/60 bg-cyan-500/10" : "border-[var(--border)] bg-[var(--card)]"}`}>
              <div className="flex items-center justify-between gap-3">
                ...
                <div className="flex gap-2">
                  <Link className="rounded-md border border-[var(--border)] px-3 py-2 text-sm text-[var(--text)]" href={`/workspace/dashboard?projectId=${project.id}`}>
                    Open Dashboard
                  </Link>
                  <Link className="rounded-md border border-[var(--border)] px-3 py-2 text-sm text-[var(--text)]" href={`/workspace/alerts?projectId=${project.id}`}>
                    Open Alerts
                  </Link>
                </div>
              </div>
